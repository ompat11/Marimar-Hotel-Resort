    ...
    <div class="col">
        ...
        <div class="card-columns">
            ...
            foreach ($cur as $result) {
                ...
                // Ensure the query returned valid results before proceeding
                $stats = $mydb->executeQuery();
                if ($stats && $rows = mysqli_fetch_assoc($stats)) {
                    $status = $rows['STATUS'];
                } else {
                    $status = null; // Handle no status available
                }

                if ($resNum == 0) {
                    if ($status == 'Confirmed') {
                        $btn = '<div style="margin-top:10px; color: rgba(0,0,0,1); font-size:16px;"><strong>Fully Reserved!</strong></div>';
                        $img_title = '<figcaption class="img-title-active"><h5>Reserved!</h5></figcaption>';
                    } elseif ($status == 'Checkedin') {
                        $btn = '<div style="margin-top:10px; color: rgba(0,0,0,1); font-size:16px;"><strong>Fully Booked!</strong></div>';
                        $img_title = '<figcaption class="img-title-active"><h5>Booked!</h5></figcaption>';
                    } else {
